#!/usr/bin/env python
"""
Structure ablation: compare ResGAT (with residual skip connections) vs
the same architecture without residual paths.

Usage:
    python ablation_study/run_structure_ablation.py \
        --graph_dir data/graphs --splits_pkl data/splits.pkl --device cuda:0
"""
...
from ablation_study.models import ResGATsNoResidual
from run import load_graphs, load_splits, build_fold_indices, train_one_fold, set_seed


def main():
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    parser = argparse.ArgumentParser(description="Structure ablation")
    ...
    variants = {
        "ResGAT (with residual)": ResGATs,
        "ResGAT (no residual)": ResGATsNoResidual,
    }
    ...
    for name, cls in variants.items():
        ...
        def _model_cls(input_dim, n_classes=args.n_classes, _cls=cls):
            return _cls(input_dim, n_classes=n_classes)
        ...
        for ti in range(len(folds)):
            ...
            m = train_one_fold(
                ...
                wandb_group=f"ablation_struct_{cls.__name__}",
            )
            fold_metrics.append(m)

        accs = np.array([m["acc"] for m in fold_metrics])
        print(f"\n{name} | Acc {np.mean(accs):.2f} +/- {np.std(accs):.2f}")
