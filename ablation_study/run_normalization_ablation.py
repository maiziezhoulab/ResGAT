#!/usr/bin/env python
"""
Normalization-layer ablation: compare GraphNorm / BatchNorm / LayerNorm /
InstanceNorm inside ResGAT blocks.

Usage:
    python ablation_study/run_normalization_ablation.py \
        --graph_dir data/graphs --splits_pkl data/splits.pkl \
        --norms graph batch layer instance --device cuda:0
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Normalization ablation")
    parser.add_argument("--graph_dir", type=str, required=True)
    parser.add_argument("--splits_pkl", type=str, required=True)
    parser.add_argument("--norms", nargs="+",
                        default=["graph", "batch", "layer", "instance"])
    ...
    for norm in args.norms:
        print(f"\n{'='*60}")
        print(f"  NORM TYPE: {norm.upper()}")
        print(f"{'='*60}")

        def _model_cls(input_dim, n_classes=args.n_classes, _nt=norm):
            return ResGATsAblation(input_dim, norm_type=_nt,
                                   n_classes=n_classes)
        ...
        for ti in range(len(folds)):
            ...
            m = train_one_fold(
                ...
                wandb_group=f"ablation_norm_{norm}",
            )
            fold_metrics.append(m)

        accs = np.array([m["acc"] for m in fold_metrics])
        print(f"\n{norm} | Acc {np.mean(accs):.2f} +/- {np.std(accs):.2f}")
