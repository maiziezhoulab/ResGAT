#!/usr/bin/env python
"""
Layer-type ablation: compare GATv2 / GCN / GIN / GraphSAGE as the
message-passing operator inside ResGAT, keeping everything else fixed.

Usage:
    python ablation_study/run_layer_type_ablation.py \
        --graph_dir data/graphs --splits_pkl data/splits.pkl \
        --conv_types gatv2 gcn gin sage --device cuda:0
"""
...
def main():
    ...
    for conv_type in args.conv_types:
        print(f"\n{'='*60}")
        print(f"  CONV TYPE: {conv_type.upper()}")
        print(f"{'='*60}")

        def _model_cls(input_dim, n_classes=args.n_classes, _ct=conv_type):
            return ResGATsAblation(input_dim, conv_type=_ct,
                                   n_classes=n_classes)
        ...
        for ti in range(len(folds)):
            ...
            m = train_one_fold(
                ...
                wandb_group=f"ablation_conv_{conv_type}",
            )
            fold_metrics.append(m)

        accs = np.array([m["acc"] for m in fold_metrics])
        print(f"\n{conv_type} | Acc {np.mean(accs):.2f} +/- {np.std(accs):.2f}")
